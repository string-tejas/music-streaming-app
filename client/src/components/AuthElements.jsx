...
export const AuthSubtitle = ({ className, children, ...other }) => {
    return (
        <p className={`md:text-[17px] text-sm text-white font-light md:p-4 p-2 text-center ${className}`} {...other}>
            {children}
        </p>
    );
};
...
export const AuthButton = ({ className, dark = false, children, ...other }) => {
    if (dark) {
        return (
            <button
                className={`${className} capitalize rounded-full min-w-[200px] md:min-w-[190px] text-center p-1 md:px-6 md:py-1 font-normal text-[16px] md:text-[18px] hover:bg-[#3b82f6] hover:text-[#ffffff] hover:shadow-lg transition-all bg-[#3b82f6] border-2 border-[#3b82f6] text-white  active:border-[#5b6560] active:bg-[#287a6b] active:text-[13px] md:active:text-[16px]`}
                {...other}
            >
                {children}
            </button>
...
export const AuthFormContainer = ({ className, children, onFormSubmit, ...other }) => {
    return (
        <form
